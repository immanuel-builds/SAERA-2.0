...
class ScanService:
    ...
    def execute_and_persist(cls, scan_job_id: int) -> CanonicalIntelligence:
        ...
        scan_job.log(
            f"Target locked: {target} (Ports: {port_range}, Mode: {scan_type.capitalize()})."
        )
        ...
        scan_job.log(
            f"XML parsing completed. Identified {len(cio.services)} active services."
        )
        ...
        scan_job.log(
            f"Risk calculations completed. Posture status evaluated as: {cio.risk.level if cio.risk else 'Clear'}."
        )
    ...
    def _persist_cio(cls, scan_job: ScanJob, cio: CanonicalIntelligence):
        ...
        with transaction.atomic():
            ...
            prev_scan = (
                ScanJob.objects.filter(target=target, status="completed")
            ...
            for s in cio.services:
                ...
                if prev_scan:
                    ...
                    if prev_port:
                        first_seen_time = (
                            prev_port.first_seen or prev_port.created_at or now
                        )
            ...
            for exp in cio.exposures:
                ...
                if prev_scan:
                    prev_exp = prev_scan.vulnerabilities.filter(
                        title=exp.title, port=exp.port
                    ).first()
                    if prev_exp:
                        first_seen_time = (
                            prev_exp.first_seen or prev_exp.created_at or now
                        )
                ...
                for vuln in cio.vulnerabilities:
                    ...
                    if prev_scan:
                        prev_vuln = prev_scan.vulnerabilities.filter(
                            title=vuln.title, port=vuln.port
                        ).first()
                        if prev_vuln:
                            first_seen_time = (
                                prev_vuln.first_seen or prev_vuln.created_at or now
                            )
            ...
            scan_job.open_ports_found = len(
                [s for s in cio.services if s.state == "open"]
            )

            scan_job.vulnerabilities_found = len(cio.exposures) + len(
                getattr(cio, "vulnerabilities", [])
            )

            from apps.scanner.risk_engine import calculate_port_exposure_risk

            open_ports = [s for s in cio.services if s.state == "open"]

            base_risk = cio.risk.score if cio.risk else 0.0
            port_risk = calculate_port_exposure_risk(open_ports)

            aggregate_score = base_risk + port_risk
            aggregate_score = min(round(aggregate_score, 1), 10.0)

            scan_job.aggregate_risk_score = aggregate_score
